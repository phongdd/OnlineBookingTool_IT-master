...
namespace CheckBookingTool
{
    public partial class RefundChecking : Form
    {
        public RefundChecking()
        {
            InitializeComponent();
        ...
        private string LongXuyen_BOConnectionString = "Data Source=192.168.143.2;Initial Catalog=VISTA;User Id=sa;Password=GaLaXyNDNTTB126;";
        private string LinhTrung_BOConnectionString = "Data Source=192.168.144.2;Initial Catalog=VISTA;User Id=sa;Password=GaLaXyNDNTTB126;";
        ...
        private void CreatTable()
        {
            ...
            this.tblReconciliationNotMatch.Columns.Add("TransC_excel_strCinema", typeof(string));
            ...
            this.tblExcelTrans.Columns.Add("TransC_excel_curValue", typeof(decimal));
            this.tblExcelTrans.Columns.Add("TransC_excel_strCinema", typeof(string));
            ...
            this.tblExcelTrans.Columns.Add("TransC_curValue", typeof(decimal));
            ...
            this.tblBO_DBTrans.Columns.Add("TransC_curValue", typeof(decimal));
        ...
        private void CombineTable()
        {
            ...
            try
            {
                #region Lấy record không có trên Vista (Không có trên Vista, Có trên file excel)
                foreach (DataRow dr in this.tblExcelTrans.Rows)
                {
                    ...
                    else
                    {
                        ...
                        row["TransC_excel_dtmDateTime"] = dr["TransC_excel_dtmDateTime"];
                        row["TransC_excel_curValue"] = dr["TransC_excel_curValue"];
                        row["TransC_excel_strCinema"] = dr["TransC_excel_strCinema"];
                ...
                #endregion Lấy record không có trên Vista (Không có trên Vista, Có trên file excel)

                #region Lấy record không có trên Excel (Có trên dữ liệu Vista, không có trên Excel)
                ...
                #endregion Lấy record không có trên Excel (Có trên dữ liệu Vista, không có trên Excel)
            }
            catch (Exception exp)
            {
        ...
        private void StartBackgroundWorker()
        {
            if (backgroundWorker1.IsBusy != true)
            {
                ...
                RefundChecking RC = new RefundChecking();
            ...
            if (this.rad123Pay.Checked)
            {
                ...
                if (openFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        DataSet ds = new DataSet();
                        string sheetname = "Danhsachgiaodich";
                        ...
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++) //Dong
                        {
                            ...
                            DataRow row;
                            {
                                if (a[2].Length > 0)
                                {
                                    ...
                                    row["TransC_excel_strCinema"] = "";

                                    row["TransC_excel_curValue"] = a[10];
                                    string[] temptValue = a[10].Split(',');
                                    string transValue = string.Empty;
            ...
            else if (this.radOnePayATM.Checked)
            {
                ...
                if (openFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        ds = GetDataExcel(openFileDialog1.FileName, sheetname);// Goi chuong trinh GetDataExcel phia tren luu vao Dataset ds        
                        for (int i = 6; i < ds.Tables[0].Rows.Count; i++) //Dong
                        {
                            for (int j = 0; j < ds.Tables[0].Columns.Count; j++)//Cot 
                            {
                                a[j] = ds.Tables[0].Rows[i].ItemArray[j].ToString().Trim();
                            ...
                            DataRow row;
                            {
                                if (a[2].Length > 0)
                                {
                                    row = this.tblExcelTrans.NewRow();
                                    row["No"] = a[0];
                                    row["TransC_excel_strPaymentTransRef"] = a[4];
                                    row["TransC_excel_strType"] = a[1];
                                    row["TransC_excel_strCinema"] = "";
                                    if (IsDateTime(a[3]))
                                        row["TransC_excel_dtmDateTime"] = a[3];
                                    row["TransC_excel_curValue"] = decimal.Parse(a[8]);
                                    this.tblExcelTrans.Rows.Add(row);
                                }
                            }
                        }
                        ...
                        try
                        {
                            object totalValue = tblExcelTrans.Compute("SUM(TransC_excel_curValue)", string.Empty);
                            string sFormat = string.Format("#{0}###", ",");
                            this.txtPayConnectorTotalAmout.Text = (decimal.Parse(totalValue.ToString())).ToString(sFormat);
                        }
                        catch (Exception)
                        {
                            this.txtPayConnectorTotalAmout.Text = "0";
                        }
                        lblTotalExcelRecord.Text = tblExcelTrans.Rows.Count.ToString() + " record(s).";
                    }
                    catch (Exception exp)
                    {
                        MessageBox.Show(string.Format("Định dạng file Excel không đúng hoặc định dạng ngày tháng dữ liệu trong file không chính xác. \nSYSTEM ERROR MESSAGE: {0}", exp.Message));
                    }
                    alert.Close();
                }
            }
                ...
                if (openFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        if (this.txtSheetName.Text.Length > 0)
                            sheetname = this.txtSheetName.Text;
                        ds = GetDataExcel(openFileDialog1.FileName, sheetname);// Goi chuong trinh GetDataExcel phia tren luu vao Dataset ds        
                        for (int i = 5; i < ds.Tables[0].Rows.Count; i++) //Dong
                        {
                            for (int j = 0; j < ds.Tables[0].Columns.Count; j++)//Cot 
                            {
                                a[j] = ds.Tables[0].Rows[i].ItemArray[j].ToString().Trim();
...

                            {
                                if (a[0].Length > 0)
                                {
                                    ...
                                    row = this.tblExcelTrans.NewRow();
                                    row["No"] = indexNumber;
                                    row["TransC_excel_strPaymentTransRef"] = a[1];
                                    row["TransC_excel_strType"] = a[2];
                                    row["TransC_excel_strCinema"] = "";
                                    if (IsDateTime(a[8]))
                                        row["TransC_excel_dtmDateTime"] = a[8];

                                    row["TransC_excel_curValue"] = a[5];
                                    string[] temptValue = a[5].Split(',');
                                    string transValue = string.Empty;
                                    if (temptValue.Length > 0)
                                    {
                                        for (int index = 0; index < temptValue.Length; index++)
                ...
                if (openFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        if (this.txtSheetName.Text.Length > 0)
                            sheetname = this.txtSheetName.Text;
                        ds = GetDataExcel(openFileDialog1.FileName, sheetname);// Goi chuong trinh GetDataExcel phia tren luu vao Dataset ds        
                        for (int i = 1; i < ds.Tables[0].Rows.Count; i++) //Dong
                        {
                            for (int j = 0; j < ds.Tables[0].Columns.Count; j++)//Cot 
                            {
                                a[j] = ds.Tables[0].Rows[i].ItemArray[j].ToString().Trim();
                            ...
                            DataRow row;
                            {
                                if (a[0].Length > 0)
                                {
                                    ...
                                    string madonhang = a[0];
                                    madonhang = madonhang.Substring(1, madonhang.Length - 1);
                                    row["TransC_excel_strPaymentTransRef"] = madonhang;

                                    string macuahang = a[2];
                                    row["TransC_excel_strCinema"] = macuahang;

                                    string ngaythanhtoan = a[7];
                                    ngaythanhtoan = ngaythanhtoan.Substring(1, ngaythanhtoan.Length - 1);
                                    if (IsDateTime(ngaythanhtoan))
                                        row["TransC_excel_dtmDateTime"] = ngaythanhtoan;

                                    row["TransC_excel_strType"] = a[9];
                        ...
                        this.txtPayConnectorTotalAmout.Text = (totalTranscMoney).ToString(sFormat);
                        lblTotalExcelRecord.Text = tblExcelTrans.Rows.Count.ToString() + " records";
                    }
                    catch (Exception exp)
                    {
        ...
        private void btnRec_Click(object sender, EventArgs e)
        {
            ...
            if (this.chkAutoChecking.Checked)
            {
                ...
                else
                {
                    alert = new AlertForm();
                    alert.Show();
                    this.LoadDataFromVistaBO(1); //=> So sanh giữa file excel (cổng thanh toán) và DB
                    alert.Close();
                }
            }
            else
            {
                this.LoadDataFromVistaBO(2); //=> So sánh từng giao dịch 1 bằng cách nhập Payment_TransRef
            }
        }
        ...
        private void RefundChecking_FormClosed(object sender, System.Windows.Forms.FormClosedEventArgs e)
        {
        ...
        private void backgroundWorker1_RunWorkerCompleted(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            ...
            // Show to Grid
            dgrVistaData.DataSource = tblBO_DBTrans;
            dgrVistaData.Refresh();

            // Show total record
            lblTotalVistaRecord.Text = tblBO_DBTrans.Rows.Count.ToString() + " record(s).";

            // Tính tổng tiền Vista => show lên textbox
            try
            {
                object totalTranscMoney;
                totalTranscMoney = tblBO_DBTrans.Compute("Sum(TransC_curValue)", string.Empty);

                string sFormat = string.Format("#{0}###", ",");

                TxttotalTranscMoney.Text = (decimal.Parse(totalTranscMoney.ToString()) * 1000).ToString(sFormat);
            }
            catch
            {
                TxttotalTranscMoney.Text = "0";
            }
            // Tính tổng tiền Vista => show lên textbox
        }

        /// <summary>
        /// Lấy Dữ Liệu Từ Các BO
        /// </summary>
        ...
        private void backgroundWorker1_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            if (!cbForTesting.Checked)
            {
                ...
                string mipec_sqlCommandString, 
                    danang_sqlCommandString, 
                    cm_sqlCommandString,
                    tc_sqlCommandString,
                    pvc_sqlCommandString,
                    htp_sqlCommandString,
                    vinh_sqlCommandString,
                    hp_sqlCommandString,
                    nvq_sqlCommandString,
                    tt_sqlCommandString,
                    bmt_sqlCommandString,
                    LongXuyen_sqlCommandString,
                    LinhTrung_sqlCommandString = string.Empty;

                string mipec_whereString,
                    danang_whereString,
                    cm_whereString,
                    tc_whereString,
                    pvc_whereString,
                    ...
                    tt_whereString,
                    bmt_whereString,
                    LongXuyen_whereString,
                    LinhTrung_whereString = string.Empty;
                #endregion
                ...
                mipec_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND (TransC_strType = '0000000014' OR TransC_strType = 'R') AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                danang_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND (TransC_strType = '0000000013' OR TransC_strType = 'R') AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                cm_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND (TransC_strType = '0000000013' OR TransC_strType = 'R') AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                tc_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                pvc_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                htp_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                vinh_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                hp_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                nvq_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                tt_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}') AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM' ", fromDate, toDate);
                bmt_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}' AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM') ", fromDate, toDate);
                LongXuyen_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}' AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM') ", fromDate, toDate);
                LinhTrung_whereString = string.Format(" WHERE (TransC_dtmRealTransTime BETWEEN '{0}' AND '{1}' AND TransC_strType = 'R' AND TransC_strBKCardType <> '123PHIM') ", fromDate, toDate);

                sqlCommandString = selectString + fromString + whereString + orderByString;
                mipec_sqlCommandString = selectString + fromString + mipec_whereString + orderByString;
                danang_sqlCommandString = selectString + fromString + danang_whereString + orderByString;
                cm_sqlCommandString = selectString + fromString + cm_whereString + orderByString;
                tc_sqlCommandString = selectString + fromString + tc_whereString + orderByString;
                pvc_sqlCommandString = selectString + fromString + pvc_whereString + orderByString;
                htp_sqlCommandString = selectString + fromString + htp_whereString + orderByString;
                vinh_sqlCommandString = selectString + fromString + vinh_whereString + orderByString;
                hp_sqlCommandString = selectString + fromString + hp_whereString + orderByString;
                nvq_sqlCommandString = selectString + fromString + nvq_whereString + orderByString;
                tt_sqlCommandString = selectString + fromString + tt_whereString + orderByString;
                bmt_sqlCommandString = selectString + fromString + bmt_whereString + orderByString;
                LongXuyen_sqlCommandString = selectString + fromString + LongXuyen_whereString + orderByString;
                LinhTrung_sqlCommandString = selectString + fromString + LinhTrung_whereString + orderByString;
                #endregion

                #region Load Data To Table
                SqlConnection Curcon = new SqlConnection();
                try
                {
                    #region //NGUYEN DU LOADING
                    Curcon.ConnectionString = nd_BOConnectionString;
                    ...
                    #region //TAN BINH LOADING
                    Curcon.ConnectionString = tb_BOConnectionString;
                    ...
                    #region //KDV LOADING
                    Curcon.ConnectionString = kdv_BOConnectionString;
                    ...
                    #region //QUANG TRUNG LOADING
                    Curcon.ConnectionString = qt_BOConnectionString;
                    ...
                    #region //BEN TRE LOADING
                    Curcon.ConnectionString = bt_BOConnectionString;
                    ...
                    #region //MIPEC LONG BIEN LOADING
                    Curcon.ConnectionString = mipec_BOConnectionString;

                    this.OpenConnection(this.mipec_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(mipec_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //DA NANG LOADING
                    Curcon.ConnectionString = danang_BOConnectionString;

                    this.OpenConnection(this.danang_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(danang_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //CA MAU LOADING
                    Curcon.ConnectionString = cm_BOConnectionString;

                    this.OpenConnection(this.cm_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(cm_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //TRUNG CHANH LOADING
                    Curcon.ConnectionString = tc_BOConnectionString;

                    this.OpenConnection(this.tc_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(tc_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //PHAM VAN CHI LOADING
                    Curcon.ConnectionString = pvc_BOConnectionString;

                    this.OpenConnection(this.pvc_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(pvc_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //HUYNH TAN PHAT LOADING
                    Curcon.ConnectionString = htp_BOConnectionString;

                    this.OpenConnection(this.htp_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(htp_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //VINH LOADING
                    Curcon.ConnectionString = vinh_BOConnectionString;

                    this.OpenConnection(this.vinh_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(vinh_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //HAI PHONG LOADING
                    Curcon.ConnectionString = hp_BOConnectionString;

                    this.OpenConnection(this.hp_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(hp_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //NGUYEN VAN QUA LOADING
                    Curcon.ConnectionString = nvq_BOConnectionString;

                    this.OpenConnection(this.nvq_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(nvq_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //TRANG THI LOADING
                    Curcon.ConnectionString = tt_BOConnectionString;

                    this.OpenConnection(this.tt_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(tt_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //BUON MA THUOT LOADING
                    Curcon.ConnectionString = bmt_BOConnectionString;

                    this.OpenConnection(this.bmt_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(bmt_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //Long Xuyen LOADING
                    Curcon.ConnectionString = LongXuyen_BOConnectionString;

                    this.OpenConnection(this.LongXuyen_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(LongXuyen_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                    ...
                    #region //Linh Trung LOADING
                    Curcon.ConnectionString = LinhTrung_BOConnectionString;

                    this.OpenConnection(this.LinhTrung_BOConnectionString);
                    this.adapter.SelectCommand = new SqlCommand(LinhTrung_sqlCommandString, this.con);
                    this.adapter.SelectCommand.CommandType = CommandType.Text;
                    this.adapter.SelectCommand.CommandTimeout = 0;
                    this.tblTemp_BO_DBTrans.Rows.Clear();
                    this.adapter.Fill(this.tblTemp_BO_DBTrans);
                ...
                {
                    MessageBox.Show(Curcon.ConnectionString + "...." + exc.Message);
                }
            ...
            else
            {
                ...
                string mipec_sqlCommandString = string.Empty;
                string mipec_whereString = string.Empty;
                ...
                #region //TEST LOADING
                string Test_ConnectionString = "Data Source=10.10.115.16;Initial Catalog=VISTA;User Id=sa;Password=GaLaXyNDNTTB126;";
                this.OpenConnection(Test_ConnectionString);
                this.adapter.SelectCommand = new SqlCommand(mipec_sqlCommandString, this.con);
                this.adapter.SelectCommand.CommandType = CommandType.Text;
                this.adapter.SelectCommand.CommandTimeout = 0;
                this.tblTemp_BO_DBTrans.Rows.Clear();
        ...
        private void chkAutoChecking_CheckedChanged(object sender, EventArgs e)
        {
            if (chkAutoChecking.Checked)
            {
                EnableChange(true);
            }
            else
        ...
        private void EnableChange(bool t)
        {
            rad123Pay.Enabled = t;
            radMoMo.Enabled = t;
            radPayoo.Enabled = t;
            radOnePayATM.Enabled = t;
            radOnePayVisa.Enabled = t;
            txtSheetName.Enabled = t;
            btnImportFromExcel.Enabled = t;
            label4.Enabled = t;

            label10.Enabled = !t;
            txtPaymentBankRef.Enabled = !t;
        }
