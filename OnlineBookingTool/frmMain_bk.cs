...
namespace OnlineBookingTool
{
    public partial class frmMain : Form
    {
      ...
      //private string connectionString = "Data Source=192.168.116.250;Initial Catalog=VISTAIT_41;User Id=sa;Password=GaLaXyNDNTTB126;";
      ...
      private void Search(int startindex, int count)
      {
          ...
          else
          {
              ...
              {
                  MessageBox.Show("Chỉ được nhập một trong 2 thông tin: địa chỉ email HOẶC số CMND, Mã ngân hàng,... để tìm kiếm!");
                  this.txtInfoSearch.Focus();
              }
              else
              {
                  if (this.txtInfoSearch.Text.Length > 0)
                      whereString = string.Format(" WHERE (WebPayH_strBankRef ='{0}' Or WebPayTN_lngPayTransNumber ='{0}' OR OrderH_intVistaBookingNumber = '{0}' OR OrderH_strPhone = '{0}' OR OrderH_strZipCode = '{0}') AND (OrderH_dtmLastUpdated BETWEEN '{1}' AND '{2}')", this.txtInfoSearch.Text, "03/01/2015 12:00:00 AM", "12/31/2016 11:59:00 PM");
                  else
                      if (this.txtEmailAddress.Text.Length > 0)
                      {
                          whereString = string.Format(" WHERE (OrderH_strEmail like '%{0}%') AND (OrderH_dtmLastUpdated BETWEEN '{1}' AND '{2}')", this.txtEmailAddress.Text, "03/01/2015 12:00:00 AM", "12/31/2016 11:59:00 PM");
                      }
                      else
                          whereString = string.Format(" WHERE OrderH_dtmLastUpdated BETWEEN '{0}' AND '{1}'", "03/01/2015 12:00:00 AM", "12/31/2016 11:59:00 PM");
